            ...
            <table class="table table-bordered table-hover" id="product-dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $product->id }}</td>
                        <td>{{ $product->title }}</td>
                        <td>{{ $product->cat_info['title'] }} <sub>{{ $sub_cat_info->title ?? '' }}</sub></td>
                        <td>{{ $product->is_featured == 1 ? 'Yes' : 'No' }}</td>
                        <td>${{ $product->price }}</td>
                        <td>{{ $product->discount }}%</td>
                        <td>{{ $product->size }}</td>
                        <td>{{ $product->condition }}</td>
                        <td>{{ ucfirst($brands->title ?? 'N/A') }}</td>
                        <td>
                            @if($product->stock > 0)
                        ...
                        <td>
                            @if($product->photo)
                            <img src="{{ asset('storage/photos/' . $product->photo) }}" class="img-fluid zoom" style="max-width:80px" alt="{{ $product->title }}">
                            @else
                            <img src="{{ asset('backend/img/thumbnail-default.jpg') }}" class="img-fluid" style="max-width:80px" alt="default-thumbnail">
                            @endif
                        </td>
                        <td>
